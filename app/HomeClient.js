...
import { CLOUD_LANGUAGE_NAMES, voicesForLanguage } from '@/lib/cloud-languages';
import {
  flushVoices,
  isOfflineSupported,
  prepareVoice,
...
const MICROSOFT_VOICES = [
  ...
  { id: 'en-US-AndrewNeural', name: 'Andrew', desc: 'Calm, professional' },
  { id: 'en-US-ChristopherNeural', name: 'Christopher', desc: 'Friendly, upbeat' },
  { id: 'en-US-EmmaNeural', name: 'Emma', desc: 'Cheerful, expressive' },
...
export default function HomeClient({ featuredNews = null }) {
  ...
  const changeTier = (tier) => {
    ...
    if (tier === 'offline') {
      setSelectedVoice({ ...PIPER_VOICES[0], tier: 'offline' });
      return;
    }

    // Returning to cloud keeps the chosen language rather than snapping to English.
    const cloudDefault =
      selectedLanguage === 'English' ? MICROSOFT_VOICES[0] : voicesForLanguage(selectedLanguage)[0];
    setSelectedVoice(cloudDefault ? { ...cloudDefault, tier: 'cloud' } : DEFAULT_CLOUD_VOICE);
  };
  ...
  const getVoices = () => {
    ...
    const cloudVoices =
      selectedLanguage === 'English' ? MICROSOFT_VOICES : voicesForLanguage(selectedLanguage);
  ...
  const changeLanguage = (language) => {
    ...
    const next = language === 'English' ? MICROSOFT_VOICES[0] : voicesForLanguage(language)[0];
    if (next) {
      setSelectedVoice({ ...next, tier: 'cloud' });
    }
  };
    ...
    <div className={styles.app}>
      ...
      <div className={styles.privacyNote}>
        ...
        <p className={styles.privacyLine}>
          Cloud voices are generated by Microsoft, then dropped. Prefer that nothing leaves your
          machine? Use offline voices below.{' '}
      ...
      <section className={styles.section}>
        ...
        {selectedTier === 'cloud' && (
          <div className={styles.languageRow}>
            <label className={styles.languageLabel} htmlFor="voice-language">
              Language
            </label>
            <select
              id="voice-language"
            ...
            >
              ...
              {CLOUD_LANGUAGE_NAMES.map((language) => (
            ...
            {selectedLanguage !== 'English' && (
              <span className={styles.languageHint}>
                Paste text in {selectedLanguage} — Varterm reads, it does not translate.
              </span>
            )}
          </div>
        )}
        ...
        <div className={styles.featuresGrid}>
          ...
          <article className={styles.featureCard}>
            ...
            <p>
              ...
              <Link href="/long-form-tts">Long-form text to speech guide</Link>.
            </p>
          </article>
          <article className={styles.featureCard}>
            <div className={styles.featureIcon}>✨</div>
